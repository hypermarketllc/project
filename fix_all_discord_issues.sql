-- Comprehensive fix for all Discord integration issues
...
-- Step 4: Create the notify_discord_on_deal function with strict duplicate checking
...
BEGIN
  -- Check if notification already exists for this deal (early exit)
  ...
  -- Double-check if notification for this deal already exists (with specific webhook)
  SELECT EXISTS (
    SELECT 1 FROM discord_notifications dn
  ...
  IF notification_exists THEN
    RAISE NOTICE 'Notification for deal % with webhook % already exists, skipping', NEW.id, webhook_url_value;
    RETURN NEW;
  END IF;
