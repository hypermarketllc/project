-- Fix webhook_url ambiguity and update notification content

-- Fix the notify_discord_on_deal function to resolve ambiguous webhook_url reference
...
-- Update the test_discord_webhook function to match the changes
CREATE OR REPLACE FUNCTION test_discord_webhook(webhook_url_param TEXT)
...
BEGIN
  ...
  SELECT EXISTS (
    SELECT 1 FROM discord_notifications dn
    WHERE dn.deal_id = latest_deal_id AND dn.webhook_url = webhook_url_param
  ) INTO notification_exists;
  
  -- If notification exists, delete it to allow a new test
  IF notification_exists THEN
    DELETE FROM discord_notifications
    WHERE deal_id = latest_deal_id AND webhook_url = webhook_url_param;
  END IF;
